...
void handleClientInput(int connection_fd)
{
    ...
    if (recv(connection_fd, buffer, sizeof(buffer), 0) == -1) // read() and recv() are almost interchangeable.
    {
        send(connection_fd, "ERR.", 4, 0); // reply to sender saying we received command.
        fprintf(stderr, "%sError%s: There was an error receiving message from socket.\n", RED, RESET);
        return;
    }

    send(connection_fd, "OK.", 3, 0); // reply to sender saying we received command.

    handleArguments(buffer, connection_fd); // checks to see if argument exists and executes them.
 ...
 **/
void handleArguments(char *arguments, int fd)
{
    char *tokens[4];
    char *end;
    int i = 0;
